...
public class FetchFalkorVideoTask extends CmpTask
{
    private final PQL pql;
    
    public FetchFalkorVideoTask(final CachedModelProxy cachedModelProxy, final String s, final BrowseAgentCallback browseAgentCallback) {
        super(cachedModelProxy, browseAgentCallback);
        this.pql = PQL.create("videos", s, PQL.array("summary", "detail"));
    }
    ...
    protected void fetchResultsAndCallbackForSuccess(final BrowseAgentCallback browseAgentCallback, final CachedModelProxy$GetResult cachedModelProxy$GetResult) {
        ...
        if (falkorVideo == null || StringUtils.isEmpty(falkorVideo.getId())) {
            final StringBuilder append = new StringBuilder().append("SPY-7519: FetchFalkorVideoTask got weird movie id: ");
            String id;
            if (falkorVideo == null) {
                id = "null";
            }
            ...
            browseAgentCallback.onFalkorVideoFetched(null, CommonStatus.INT_ERR_CMP_RESP_NULL);
            return;
        }
        browseAgentCallback.onFalkorVideoFetched(falkorVideo, CommonStatus.OK);
    }
}
