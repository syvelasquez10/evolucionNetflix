...
public class FetchBBVideosRequest extends FalcorVolleyWebClientRequest<List<Billboard>>
{
    ...
    public FetchBBVideosRequest(final Context context, final BrowseWebClientCache browseCache, final LoMo mLoMo, final int fromVideo, final int toVideo, final int toSimilars, final boolean userConnectedToFacebook, final BrowseAgentCallback responseCallback) {
        ...
        this.pqlQuery5 = String.format("['lists', '%s', 'socialEvidence', {'from':%d,'to':%d}, 'socialBadge']", mLoMo.getId(), fromVideo, toVideo);
        if (Log.isLoggable("nf_service_browse_fetchbbvideosrequest", 2)) {
            Log.v("nf_service_browse_fetchbbvideosrequest", "PQL = " + this.pqlQuery1 + " " + this.pqlQuery2 + " " + this.pqlQuery3 + " " + this.pqlQuery4 + " " + this.pqlQuery5);
        }
    }
    
    public static List<Billboard> buildBBVideoList(final JsonObject jsonObject, final JsonObject jsonObject2, final int n, int i, final int n2, final boolean b, final BrowseWebClientCache browseWebClientCache) {
        ...
        while (i >= n) {
            ...
            if (jsonObject.has(string)) {
                ...
                if (jsonObject2 != null && jsonObject2.has(string)) {
                    billboardDetails.socialBadge = FalcorParseUtils.getPropertyObject(jsonObject2.getAsJsonObject(string), "socialBadge", SocialBadge.class);
                }
    ...
    @Override
    protected List<Billboard> parseFalcorResponse(final String s) {
        if (Log.isLoggable("nf_service_browse_fetchbbvideosrequest", 2)) {
        ...
        try {
            ...
            return buildBBVideoList(asJsonObject.getAsJsonObject("videoEvidence"), asJsonObject.getAsJsonObject("socialEvidence"), this.fromVideo, this.toVideo, this.toSimilars, this.userConnectedToFacebook, this.browseCache);
        }
        catch (Exception ex) {
            Log.v("nf_service_browse_fetchbbvideosrequest", "String response to parse = " + s);
