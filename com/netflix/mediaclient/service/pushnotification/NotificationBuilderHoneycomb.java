...
public class NotificationBuilderHoneycomb extends NotificationBuilder
{
    public static void createNotification(final Context context, final Payload payload, final ImageLoader imageLoader, final int n) {
        ...
        final String title = payload.getTitle(context.getString(2131230902));
        ...
        notification$Builder.setSmallIcon(2130837754);
        ...
        while (true) {
            if (StringUtils.isNotEmpty(payload.sound) && NotificationBuilder.isSoundEnabled(context)) {
                ...
                send(context, notification$Builder, n);
                return;
            }
            continue;
        }
    ...
    @SuppressLint({ "NewApi" })
    private static void send(final Context context, final Notification$Builder notification$Builder, final int n) {
        NotificationBuilder.sendNotification(context, notification$Builder.getNotification(), n);
    }
}
