...
class UserActionLoggingImpl$8 implements Runnable
{
    ...
    UserActionLoggingImpl$8(final UserActionLoggingImpl this$0, final long val$requestId, final IClientLogging$CompletionReason val$reason, final UIError val$error, final DataContext val$dataContext) {
        this.this$0 = this$0;
    ...
    public void run() {
        ...
        final SearchSession searchSession = this.this$0.mSearchSessions.get(this.val$requestId);
        if (searchSession == null) {
            Log.w("nf_log", "Search session does NOT exist for " + this.val$requestId);
            return;
        }
        final SearchEndedEvent endedEvent = searchSession.createEndedEvent(this.val$reason, this.val$error);
        if (endedEvent == null) {
            Log.d("nf_log", "Search session still waits on session id, do not post at this time.");
            return;
        }
        this.this$0.populateEvent(endedEvent, this.val$dataContext, searchSession.getView());
        this.this$0.mEventHandler.removeSession(searchSession);
