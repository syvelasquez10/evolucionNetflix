...
class ApmLoggingImpl$13 implements Runnable
{
    ...
    ApmLoggingImpl$13(final ApmLoggingImpl this$0, final String val$sessionId, final IClientLogging$CompletionReason val$completionReason, final HttpResponse val$httpResponse, final Error val$error, final DataContext val$dataContext, final IClientLogging$ModalView val$ui) {
        this.this$0 = this$0;
    ...
    @Override
    public void run() {
        final UIDataRequestSession uiDataRequestSession = this.this$0.mDataRequests.remove(this.val$sessionId);
        if (uiDataRequestSession == null) {
            if (Log.isLoggable("nf_log_apm", 3)) {
                Log.d("nf_log_apm", "UI data request session does not exist (probably due to sampling) - sessionId: " + this.val$sessionId);
            }
            return;
        }
        this.this$0.mEventHandler.removeSession(uiDataRequestSession);
        Log.d("nf_log_apm", "Data request session end event posting...");
        final UIDataRequestSessionEndedEvent endedEvent = uiDataRequestSession.createEndedEvent(this.val$completionReason, this.val$httpResponse, this.val$error);
