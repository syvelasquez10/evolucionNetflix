...
public class PServiceWidgetAgent extends PServiceAgent implements PServiceAgent$PServiceWidgetAgentInterface
{
    ...
    private RemoteViews buildWidgetStaticImageRemoteView(final Context context, final int n) {
        ...
        remoteViews.setImageViewResource(2131690148, 2130837910);
        remoteViews.setViewVisibility(2131690156, 0);
        remoteViews.setViewVisibility(2131690157, 0);
        remoteViews.setTextViewText(2131690156, context.getText(2131231245));
    ...
    private int getLayoutId(final Context context, int n) {
        if (this.isWidgetOneCellHigh(context, n)) {
            Log.d("nf_preapp_widgetagent", String.format("using preapp_widget_align_height %d", 2130903239));
            return 2130903239;
        }
        if (this.toAlignByWidth(context, n)) {
            Log.d("nf_preapp_widgetagent", String.format("using preapp_widget_vertical %d", 2130903241));
            return 2130903241;
        }
        ...
        Log.d("nf_preapp_widgetagent", String.format("using %d, preapp_widget_align_height=%d, preapp_width:%d, (widthInCell:%d) preapp_width_2cell:%d", n, 2130903239, 2130903242, cellFromDp, 2130903243));
        return n;
    }
    ...
    private int getNumberOfImages(final Context context, int widgetHeight, final int n) {
        int n2 = 3;
        if (2130903239 == widgetHeight || 2130903243 == widgetHeight) {
            n2 = 1;
        }
        else if (2130903241 != widgetHeight) {
            if (2130903242 != widgetHeight) {
                return 1;
            }
    ...
    private String getPreAppAdString(final Context context, final int n) {
        ...
        switch (n % 3) {
            ...
            case 0: {
                s = context.getString(2131231243);
                break;
            }
            case 1: {
    ...
    private String getVideoTitle(final Context context, final PDiskData$ListType pDiskData$ListType, final PVideo pVideo) {
        if (PDiskData$ListType.CW.equals(pDiskData$ListType) && VideoType.SHOW.equals(pVideo.videoType)) {
            return context.getString(2131231151, new Object[] { pVideo.title, pVideo.playableSeasonNumAbbrLabel, pVideo.playableEpisodeNumber });
        }
        return pVideo.title;
    }
