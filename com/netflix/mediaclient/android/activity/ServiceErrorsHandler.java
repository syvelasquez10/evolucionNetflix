...
public class ServiceErrorsHandler
{
    ...
    private static boolean handleAppUpdateNeeded(final Activity activity, final boolean b) {
        ...
        if (!b) {
            updateDialog$Builder.setMessage(2131492997);
            updateDialog$Builder.setCancelable(false);
            updateDialog$Builder.setNegativeButton(2131493114, (DialogInterface$OnClickListener)new ServiceErrorsHandler$1(activity));
        }
        else {
            updateDialog$Builder.setMessage(2131492998);
        }
        updateDialog$Builder.setPositiveButton(2131492994, (DialogInterface$OnClickListener)new ServiceErrorsHandler$2(activity));
    ...
    public static boolean handleManagerResponse(final Activity activity, final Status status) {
        ...
        switch (ServiceErrorsHandler$4.$SwitchMap$com$netflix$mediaclient$StatusCode[statusCode.ordinal()]) {
            default: {
                provideDialog(activity, activity.getString(2131493256) + " (" + statusCode.getValue() + ")");
                b = true;
                return b;
            }
            case 1: {
            ...
            case 4: {
                provideDialog(activity, activity.getString(2131493126));
                return true;
            }
            case 5:
            case 6: {
                provideDialog(activity, activity.getString(2131493231));
                return true;
            }
            case 7:
            case 8:
            case 9: {
                provideDialog(activity, activity.getString(2131493255) + " (" + statusCode.getValue() + ")");
                return true;
            }
        }
    }
    ...
    private static void provideDialog(final Activity activity, final String message) {
        new AlertDialog$Builder((Context)activity).setCancelable(false).setMessage((CharSequence)message).setPositiveButton(2131492994, (DialogInterface$OnClickListener)new ServiceErrorsHandler$3(activity)).show();
    }
}
