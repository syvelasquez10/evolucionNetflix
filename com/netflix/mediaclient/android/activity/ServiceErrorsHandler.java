...
public class ServiceErrorsHandler
{
    ...
    private static String buildNetflixConnectivityErrorMessage(final Activity activity, final StatusCode statusCode) {
        return activity.getString(2131296855) + " (" + statusCode.getValue() + ")";
    }
    ...
    private static boolean handleAppUpdateNeeded(final Activity activity, final boolean b) {
        ...
        if (!b) {
            updateDialog$Builder.setMessage(2131296538);
            ...
            updateDialog$Builder.setMessage(2131296677);
        }
        updateDialog$Builder.setPositiveButton(2131296724, (DialogInterface$OnClickListener)new ServiceErrorsHandler$2(activity));
    ...
    public static boolean handleManagerResponse(final Activity activity, final Status status) {
        ...
        switch (ServiceErrorsHandler$7.$SwitchMap$com$netflix$mediaclient$StatusCode[statusCode.ordinal()]) {
            ...
            case 5: {
                provideDialog(activity, activity.getString(2131296817));
                return true;
            }
            case 6:
            case 7: {
                provideDialog(activity, activity.getString(2131296636));
                return true;
            }
            case 8: {
                provideDialog(activity, activity.getString(2131296643));
                return true;
            }
            case 9:
            case 10:
            case 11: {
                provideDialog(activity, activity.getString(2131296846) + " (" + statusCode.getValue() + ")");
                return true;
            }
            case 12: {
    ...
    private static void provideDialog(final Activity activity, final String message) {
        new AlertDialog$Builder((Context)activity).setCancelable(false).setMessage(message).setPositiveButton(2131296724, (DialogInterface$OnClickListener)new ServiceErrorsHandler$3(activity)).show();
    }
    
    private static void provideDialogWithHelpButton(final Activity activity, final String message) {
        new AlertDialog$Builder((Context)activity).setCancelable(false).setMessage(message).setPositiveButton(2131296724, (DialogInterface$OnClickListener)new ServiceErrorsHandler$5(activity)).setNegativeButton(2131296668, (DialogInterface$OnClickListener)new ServiceErrorsHandler$4(activity)).show();
    }
