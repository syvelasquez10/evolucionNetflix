...
class BaseNotification$1onBoxArtFetched implements ImageLoader$ImageLoaderListener
{
    ...
    public void onResponse(final Bitmap bitmap, final String s) {
        if (bitmap != null) {
            final int color = this.val$context.getResources().getColor(2131558518);
            final NotificationCompat$BigPictureStyle bigPicture = new NotificationCompat$BigPictureStyle().bigPicture(bitmap);
            final NotificationCompat$Builder setColor = new NotificationCompat$Builder(this.val$context).setSmallIcon(2130837723).setLargeIcon(this.mSocialProfile).setContentTitle(this.val$notificationSummary.getFriendProfile().getFullName()).setStyle(bigPicture).setColor(color);
