...
public class EpisodesFrag extends NetflixDialogFrag implements ErrorWrapper$Callback, ManagerStatusListener, DetailsActivity$Reloader, ServiceManagerProvider, VideoDetailsViewGroup$VideoDetailsViewGroupProvider, MdxMiniPlayerFrag$MdxMiniPlayerDialog
{
    ...
    public EpisodesFrag() {
        ...
        this.episodeRefreshReceiver = new EpisodesFrag$5(this);
        this.errorCallback = new EpisodesFrag$6(this);
    }
    ...
    public static NetflixDialogFrag create(final String s, final String s2, final boolean b) {
        ...
        episodesFrag.setStyle(1, 2131558696);
    ...
    private void fetchShowDetails() {
        ...
        if (this.getNetflixActivity() == null) {
            Log.w("EpisodeListFrag", "Activity is null - can't get show details");
            return;
        }
        this.isLoading = true;
        this.requestId = System.nanoTime();
    ...
    private void setItemChecked(final int n) {
        if (this.recyclerView != null && this.episodesAdapter != null) {
            this.recyclerView.post((Runnable)new EpisodesFrag$3(this, n));
        }
    }
    
    private void showLoadingView() {
        Log.v("EpisodeListFrag", "Showing loading view");
    ...
    protected void addSpinnerToDetailsGroup() {
        ...
        this.detailsViewGroup.addView((View)this.spinnerViewGroup);
    }
    ...
    protected ViewGroup createSeasonsSpinnerGroup() {
        ...
        (this.spinner = spinner).setOnItemSelectedListener((AdapterView$OnItemSelectedListener)new EpisodesFrag$2(this));
    ...
    public void onCreate(final Bundle bundle) {
        ...
        if (bundle != null) {
            ...
            if (Log.isLoggable("EpisodeListFrag", 2)) {
                Log.v("EpisodeListFrag", "Restored season index as: " + this.currSeasonIndex + ", episode: " + this.initialEpisodeIndex);
            }
        }
    ...
    public void onSaveInstanceState(final Bundle bundle) {
        ...
        final int n = this.getCheckedItemPosition() - 1;
        if (Log.isLoggable("EpisodeListFrag", 2)) {
            Log.v("EpisodeListFrag", "Saving season index as: " + selectedItemPosition + ", episode index: " + n);
        }
        bundle.putInt("extra_season_index", selectedItemPosition);
        bundle.putInt("extra_episode_index", n);
    }
    
    protected void postSetSpinnerSelectionRunnable() {
        this.handler.post((Runnable)new EpisodesFrag$4(this));
    }
    
    @Override
    public void reload() {
        Log.v("EpisodeListFrag", "reload()");
        this.fetchShowDetails();
    }
    ...
    protected void setupDetailsPageParallaxScrollListener() {
        if (this.getActivity() != null && this.getRecyclerView() != null && this.getRecyclerView().getContext() instanceof NetflixActivity && (DeviceUtils.isNotTabletByContext((Context)this.getActivity()) || DeviceUtils.isPortrait((Context)this.getActivity()))) {
            ...
            if (netflixActionBar != null) {
                ...
                this.getRecyclerView().setOnScrollListener(new DetailsPageParallaxScrollListener(this.spinner, this.getRecyclerView(), (View)this.detailsViewGroup.getHeroImage(), null, this.getActivity().getResources().getDrawable(2130837560)));
            }
        }
    }
    ...
    public void updateEpisodeSelection() {
        ...
        if (this.selectedEpisodeIndex == -1 && this.episodesAdapter != null && this.showDetails != null) {
            ...
            if (Log.isLoggable("EpisodeListFrag", 2)) {
                Log.v("EpisodeListFrag", "currEpisodeId: " + currentEpisodeId + ", adapter count: " + this.episodesAdapter.getItemCount());
            }
            for (int i = 0; i < this.episodesAdapter.getItemCount(); ++i) {
                ...
                if (item != null && StringUtils.safeEquals(item.getId(), currentEpisodeId)) {
                    if (Log.isLoggable("EpisodeListFrag", 2)) {
                        Log.v("EpisodeListFrag", "found selected episode, i: " + i);
                    }
        ...
        if (Log.isLoggable("EpisodeListFrag", 2)) {
            Log.v("EpisodeListFrag", "selectedEpIndex: " + this.selectedEpisodeIndex);
        }
