...
public class LastTime extends PlaycardSection
{
    ...
    public void setLastTimeState(final boolean b) {
        ...
        final BottomPanel bottomPanel = context.getScreen().getBottomPanel();
        ...
        if (this.lastTimeLabel != null) {
            final String formatTime = bottomPanel.formatTime(bottomPanel.getTimeline().getProgress());
            if (Log.isLoggable("screen", 3)) {
                Log.d("screen", "setLastTimeState::Sets Last time update to: " + formatTime);
            }
            this.lastTimeLabel.setText((CharSequence)formatTime);
        }
