...
public final class CoppolaUtils
{
    private static final String TAG = "CoppolaUtils";
    ...
    public static void injectPlayerFragmentIfNeeded(final Activity activity, final String s, final VideoType videoType, final PlayContext playContext, final ServiceManager serviceManager, final Status status) {
        if (shouldInjectPlayerFragment((Context)activity) && !AndroidUtils.isActivityFinishedOrDestroyed(activity)) {
            ...
            new Handler().postDelayed((Runnable)new CoppolaUtils$1(activity, playerFragment, serviceManager, status), 2000L);
        }
    }
    
    public static boolean isAudioOn(final Context context) {
        return PersistentConfig.getCoppola1ABTestCell(context) != ABTestConfig$Cell.CELL_SEVEN;
    }
    
    public static boolean isAutoplay(final Context context) {
        return PersistentConfig.getCoppola1ABTestCell(context).ordinal() > ABTestConfig$Cell.CELL_THREE.ordinal();
    }
    
    public static boolean isBrowsePlay(final Activity activity) {
        if (Log.isLoggable()) {
            Log.v("CoppolaUtils", "isBrowsePlay() - CoppolaUtils.isCoppolaContext(activity): " + isCoppolaContext((Context)activity) + ", DeviceUtils.isPortrait(activity): " + DeviceUtils.isPortrait((Context)activity) + ", CoppolaUtils.isAutoplay(activity): " + isAutoplay((Context)activity));
        }
    ...
    public static boolean isCoppolaExperience(final Context context) {
        return PersistentConfig.getCoppola1ABTestCell(context).ordinal() > ABTestConfig$Cell.CELL_ONE.ordinal() && PersistentConfig.getCoppola1ABTestCell(context).ordinal() < ABTestConfig$Cell.CELL_NINE.ordinal();
    }
    ...
    public static boolean isNewPlayerExperience(final Context context) {
        return PersistentConfig.getCoppola1ABTestCell(context).ordinal() > ABTestConfig$Cell.CELL_TWO.ordinal() && PersistentConfig.getCoppola1ABTestCell(context).ordinal() < ABTestConfig$Cell.CELL_NINE.ordinal();
    }
    ...
    public static void launchCoppolaDetails(final NetflixActivity netflixActivity, final Asset asset, final boolean b, final int n) {
        ...
        if (n > -1) {
            if (n < asset.getDuration()) {
                ...
                Log.w("CoppolaUtils", "Start time parameter was ignored since it exceeds the total duration.");
            }
        }
        else {
    ...
    public static boolean showCountdownTimer(final Context context) {
        return PersistentConfig.getCoppola1ABTestCell(context) == ABTestConfig$Cell.CELL_EIGHT;
    }
