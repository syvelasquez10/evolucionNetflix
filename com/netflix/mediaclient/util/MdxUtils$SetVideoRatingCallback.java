...
public class MdxUtils$SetVideoRatingCallback extends LoggingManagerCallback
{
    ...
    public void onVideoRatingSet(final UserRating userRating, final Status status) {
        ...
        if (status.isError()) {
            ...
            Toast.makeText((Context)this.activity, 2131165437, 1).show();
            Log.d("MdxUtils", "Report rate action ended");
            UserActionLogUtils.reportRateActionEnded((Context)this.activity, IClientLogging$CompletionReason.failed, ConsolidatedLoggingUtils.createUIError(status, this.activity.getString(2131165437), ActionOnUIError.displayedError), null, (int)this.rating);
            return;
        }
        Log.v("MdxUtils", "onVideoRatingSet: Rating has been updated to: " + userRating.getUserRating());
        Toast.makeText((Context)this.activity, 2131165517, 1).show();
