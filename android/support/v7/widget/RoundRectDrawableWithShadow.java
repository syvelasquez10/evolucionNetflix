...
class RoundRectDrawableWithShadow extends Drawable
{
    ...
    void setShadowSize(float mRawShadowSize, float mRawMaxShadowSize) {
        if (mRawShadowSize < 0.0f) {
            throw new IllegalArgumentException("Invalid shadow size " + mRawShadowSize + ". Must be >= 0");
        }
        if (mRawMaxShadowSize < 0.0f) {
            throw new IllegalArgumentException("Invalid max shadow size " + mRawMaxShadowSize + ". Must be >= 0");
        }
        final float n = this.toEven(mRawShadowSize);
        mRawMaxShadowSize = this.toEven(mRawMaxShadowSize);
        mRawShadowSize = n;
        if (n > mRawMaxShadowSize) {
        ...
        this.mShadowSize = (int)(mRawShadowSize * 1.5f + this.mInsetShadow + 0.5f);
